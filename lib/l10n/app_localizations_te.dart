// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Telugu (`te`).
class AppLocalizationsTe extends AppLocalizations {
  AppLocalizationsTe([String locale = 'te']) : super(locale);

  @override
  String get appTitle => 'మన డ్రైవర్';

  @override
  String get selectLanguageTitle => 'మీ భాషను ఎంచుకోండి.';

  @override
  String get selectLanguageSubtitle => 'మీ ఎంపిక చేసిన భాషలో కొనసాగించడానికి ట్యాప్ చేయండి.';

  @override
  String get chooseLanguage => 'భాషను ఎంచుకోండి';

  @override
  String get continueButton => 'కొనసాగించండి';

  @override
  String get loginTitle => 'మీ మొబైల్ నంబర్‌ను నమోదు చేయండి.';

  @override
  String get loginSubtitle => 'ప్రారంభించడానికి మీ మొబైల్ నంబర్‌ను నమోదు చేయండి, ధృవీకరణ కోసం మేము మీకు ఓటీపీని పంపుతాము.';

  @override
  String get checking => 'తనిఖీ జరుగుతోంది...';

  @override
  String get sendOtp => 'ఓటీపీ పంపండి';

  @override
  String get noAccount => 'మీకు ఖాతా లేదా? ';

  @override
  String get register => 'నమోదు చేయండి';

  @override
  String get enterMobileNumber => 'దయచేసి మొబైల్ నంబర్‌ను నమోదు చేయండి';

  @override
  String get mobileNumber => 'మొబైల్ నంబర్';

  @override
  String get invalidOtp => 'చెల్లని OTP. దయచేసి మళ్లీ ప్రయత్నించండి.';

  @override
  String get otpTitle => 'మీ OTPని నమోదు చేయండి';

  @override
  String get otpSentTo => 'ఓటీపీ పంపబడింది ';

  @override
  String get otpAutoFill => ' ఈ ఓటీపీ ఆటో ఫిల్ అవుతుంది';

  @override
  String get didNotReceiveOtp => 'మీకు ఓటీపీ రాలేదా?';

  @override
  String get resendOtp => 'ఓటీపీ మళ్లీ పంపండి';

  @override
  String get verifyOtp => 'ఓటీపీని ధృవీకరించండి';

  @override
  String get bottomNavHome => 'హోమ్';

  @override
  String get bottomNavMyRides => 'నా ప్రయాణాలు';

  @override
  String get bottomNavTransactions => 'లావాదేవీలు';

  @override
  String get bottomNavMenu => 'మెను';

  @override
  String get greeting => 'నమస్కారం';

  @override
  String get pickupLocation => 'పికప్ స్థానం';

  @override
  String get enterPickupLocation => 'పికప్ లొకేషన్ నమోదు చేయండి';

  @override
  String get dropLocation => 'డ్రాప్ స్థానం';

  @override
  String get enterDropLocation => 'డ్రాప్ లొకేషన్ నమోదు చేయండి';

  @override
  String get dropLocation2 => 'డ్రాప్ స్థానం 2';

  @override
  String get enterDropLocation2 => 'డ్రాప్ లొకేషన్ 2 నమోదు చేయండి';

  @override
  String get bookADriver => 'డ్రైవర్ బుక్ చేయండి';

  @override
  String get myVehicles => 'నా వాహనాలు';

  @override
  String get viewVehicles => 'వాహనాలను వీక్షించండి';

  @override
  String get menumyAddress => 'నా చిరునామా';

  @override
  String get menuFavDrivers => 'ఇష్టమైన డ్రైవర్లు';

  @override
  String get menuUpdateMobileNumber => 'మొబైల్ నంబర్‌ని అప్‌డేట్ చేయండి';

  @override
  String get menuAppLanguage => 'యాప్ భాష';

  @override
  String get menuOffers => 'ఆఫర్లు';

  @override
  String get menuReferaFriend => 'స్నేహితుడిని సూచించండి';

  @override
  String get menuTC => 'నిబంధనలు & షరతులు';

  @override
  String get menuHelpSupport => 'సహాయం & మద్దతు';

  @override
  String get menuCancelPolicy => 'రద్దు విధానం';

  @override
  String get menuAbtMD => 'మన డ్రైవర్ గురించి';

  @override
  String get menuDeleteAccount => 'ఖాతాను తొలగించండి';

  @override
  String get menuLogout => 'లాగ్అవుట్';

  @override
  String get menuEnterMobile => 'మొబైల్‌ని నమోదు చేయండి';

  @override
  String get menuEnterOTP => 'OTPని నమోదు చేయండి';

  @override
  String get menuDontRecieved => 'అందలేదా? ';

  @override
  String get menuResend => 'మళ్లీ పంపండి';

  @override
  String get menuCancel => 'రద్దు చేయి';

  @override
  String get menuUpdate => 'నవీకరించు';

  @override
  String get menuDelete => 'తొలగించు';

  @override
  String get menuSave => 'సేవ్ చేయండి';

  @override
  String get menuSaving => 'సేవ్ చేస్తోంది...';

  @override
  String get address => 'చిరునామా,';

  @override
  String get dummy_adres => 'St.No.98, మెయిన్ రోడ్, JLN హౌస్ దగ్గర సెరిలింగంపల్లి, కొండాపూర్, 500084';

  @override
  String get add_new_Address => 'కొత్త చిరునామాను జోడించండి';

  @override
  String get fav_dummy_text => 'మీకు ప్రస్తుతం డ్రైవర్లు ఇష్టం లేదు, దయచేసి కొంత సమయం తర్వాత ప్రయత్నించండి.';

  @override
  String get offer_dummy_text => 'మీకు ప్రస్తుతం ఆఫర్‌లు లేవు. దయచేసి తర్వాత మళ్లీ ప్రయత్నించండి.';

  @override
  String get rat_Txt1 => 'మీ స్నేహితుడిని సంప్రదించి 100/- రివార్డులు పొందండి.';

  @override
  String get rat_Txt2 => 'స్నేహితుడికి ఆహ్వానం పంపండి';

  @override
  String get rat_Txt3 => 'మీ స్నేహితుడు సైన్అప్';

  @override
  String get rat_Txt4 => 'మీ స్నేహితుడు మొదట రైడ్ బుక్ చేసుకున్నప్పుడు మీ ఇద్దరికీ నగదు లభిస్తుంది.';

  @override
  String get rat_Txt5 => 'ఆహ్వానాన్ని పంపండి';

  @override
  String get tDt1 => '1. పరిచయం';

  @override
  String get tD_D1 => 'మా యాప్‌కు స్వాగతం. ఈ అప్లికేషన్‌ను ఉపయోగించడం ద్వారా, మీరు ఈ క్రింది నిబంధనలు మరియు షరతులకు అంగీకరిస్తున్నారు.';

  @override
  String get tDt2 => '2. వినియోగదారు బాధ్యతలు';

  @override
  String get tD_D2 => 'వినియోగదారులు అందించిన సమాచారం అంతా ఖచ్చితమైనదని మరియు సేవను దుర్వినియోగం చేయకూడదని నిర్ధారించుకోవాలి.';

  @override
  String get tDt3 => '3. ఖాతా భద్రత';

  @override
  String get tD_D3 => 'మీ ఖాతా ఆధారాల గోప్యతను కాపాడుకునే బాధ్యత మీదే.';

  @override
  String get tDt4 => '4. డేటా గోప్యత';

  @override
  String get tD_D4 => 'మేము మీ గోప్యతను గౌరవిస్తాము. మీ డేటా సురక్షితంగా నిల్వ చేయబడుతుంది మరియు మా గోప్యతా విధానం ప్రకారం నిర్వహించబడుతుంది.';

  @override
  String get tDt5 => '5. మేధో సంపత్తి';

  @override
  String get tD_D5 => 'అనువర్తనంలోని మొత్తం కంటెంట్ కాపీరైట్ ద్వారా రక్షించబడింది మరియు అనుమతి లేకుండా తిరిగి ఉపయోగించబడకపోవచ్చు.';

  @override
  String get tDt6 => '6. సేవా మార్పులు';

  @override
  String get tD_D6 => 'నోటీసు లేకుండా సేవను సవరించడానికి లేదా నిలిపివేయడానికి మాకు హక్కు ఉంది.';

  @override
  String get tDt7 => '7. రద్దు';

  @override
  String get tD_D7 => 'మీరు ఇక్కడ పేర్కొన్న ఏవైనా నిబంధనలను ఉల్లంఘిస్తే, మేము మీ యాక్సెస్‌ను తాత్కాలికంగా నిలిపివేయవచ్చు లేదా ముగించవచ్చు.';

  @override
  String get tDt8 => '8. మూడవ పక్షం లింక్‌లు';

  @override
  String get tD_D8 => 'మేము మూడవ పక్ష సైట్‌లకు లింక్‌లను చేర్చవచ్చు. వాటి కంటెంట్‌కు మేము బాధ్యత వహించము.';

  @override
  String get tDt9 => '9. పాలక చట్టం';

  @override
  String get tD_D9 => 'ఈ నిబంధనలు మీ దేశం లేదా ప్రాంతం యొక్క చట్టాలకు అనుగుణంగా నిర్వహించబడతాయి.';

  @override
  String get hS_t1 => 'ఈరోజు నేను మీకు ఎలా సహాయం చేయగలను?';

  @override
  String get hS_t2 => 'సంప్రదింపు వివరాలు';

  @override
  String get hS_t3 => 'కాల్ కోసం';

  @override
  String get hS_t4 => 'మెయిల్ పంపండి';

  @override
  String get cP_q1 => '1. ఉచిత రద్దు విండో';

  @override
  String get cP_a1 => 'మీరు మీ బుకింగ్‌ను నిర్ధారించిన 5 నిమిషాల్లోపు ఎటువంటి ఛార్జీలు లేకుండా రద్దు చేసుకోవచ్చు.';

  @override
  String get cP_q2 => '2. 5 నిమిషాల తర్వాత రద్దు';

  @override
  String get cP_a2 => '• ఈ క్రింది సందర్భాలలో 50 రద్దు రుసుము వసూలు చేయబడుతుంది:';

  @override
  String get cP_a22 => '• డ్రైవర్ ఇప్పటికే ప్రయాణాన్ని ప్రారంభించాడు.';

  @override
  String get cP_a23 => '• డ్రైవర్ మీ స్థానంలో 10 కంటే ఎక్కువసేపు వేచి ఉన్నాడు';

  @override
  String get cP_q3 => '3. నో-షో పాలసీ (కస్టమర్ హాజరు కాలేదు)';

  @override
  String get cP_a3 => '15 నిమిషాల తర్వాత కూడా కస్టమర్ పికప్ పాయింట్ వద్ద అందుబాటులో లేకుంటే, ట్రిప్ స్వయంచాలకంగా రద్దు చేయబడుతుంది. #100 నో-షోగా ఛార్జ్ చేయబడుతుంది.';

  @override
  String get cP_q4 => '4. డ్రైవర్ రద్దు';

  @override
  String get cP_a4 => 'ఒక డ్రైవర్ అంగీకరించిన తర్వాత రద్దు చేస్తే, మేము మరొక డ్రైవర్‌ను తిరిగి నియమిస్తాము. డ్రైవర్లు పదే పదే రద్దు చేస్తే జరిమానాలు మరియు సస్పెన్షన్ విధించబడతాయి.';

  @override
  String get cP_q5 => '5. వాపసు కాలక్రమం';

  @override
  String get cP_a5 => 'మీరు ఆన్‌లైన్‌లో చెల్లించినట్లయితే, అర్హత కలిగిన వాపసు 3-5 పని దినాలలో ప్రాసెస్ చేయబడుతుంది.';

  @override
  String get mDdisk => 'మన డ్రైవర్ - మీ వాహనం, మా డ్రైవర్!\nమీకు అవసరమైనప్పుడు ప్రొఫెషనల్, ధృవీకరించబడిన డ్రైవర్లను బుక్ చేసుకోవడానికి మన డ్రైవర్ మీ విశ్వసనీయ వేదిక. ఇది వన్-వే రైడ్ అయినా, రౌండ్ ట్రిప్ అయినా, గంటవారీ బుకింగ్ అయినా లేదా అవుట్‌స్టేషన్ ప్రయాణం అయినా - మేము మీకు రక్షణ కల్పించాము.';

  @override
  String get dA_t1 => 'హెచ్చరిక';

  @override
  String get dA_t2 => 'మీరు ఖచ్చితంగా మీ ఖాతాను తొలగించాలనుకుంటున్నారా?';

  @override
  String get profile => 'ప్రొఫైల్';

  @override
  String get p_firstName => 'మొదటి పేరు';

  @override
  String get p_lastName => 'చివరి పేరు';

  @override
  String get p_email => 'ఇమెయిల్ చిరునామా';

  @override
  String get p_phoneNumner => 'ఫోన్ నంబర్';

  @override
  String get p_verified => 'ధృవీకరించబడింది';

  @override
  String get p_editProfile => 'ప్రొఫైల్‌ని సవరించండి';

  @override
  String get home_viewoffers => 'ఆఫర్‌లను వీక్షించండి';

  @override
  String get home_watch => 'చూడండి & నేర్చుకోండి';

  @override
  String get home_prem => 'ఆత్మవిశ్వాసంతో డ్రైవ్ చేయండి';

  @override
  String get home_india => 'భారతదేశంలో తయారు చేయబడింది';

  @override
  String get enterOtp => 'మీ OTP ను నమోదు చేయండి';

  @override
  String get otpSent => ' OTP పంపబడింది !!';

  @override
  String get noOtp => 'మీకు OTP రాలేదా? ';

  @override
  String get resentOtp => 'OTP మళ్లీ పంపండి';

  @override
  String get fcmotptitle => 'తిరిగి స్వాగతం, కెప్టెన్!';

  @override
  String get fcmotpbody => 'మీరు ఇప్పుడు లాగిన్ అయ్యారు, సిద్ధంగా ఉన్నారు.';

  @override
  String get bookings => 'బుకింగ్స్';

  @override
  String get earnings => 'ఆదాయం';

  @override
  String get help => 'సహాయం';

  @override
  String get namaskaram => 'నమస్కారం';

  @override
  String get online => 'ఆన్‌లైన్';

  @override
  String get offline => 'ఆఫ్‌లైన్';

  @override
  String get myEarnings => 'నా ఆదాయం';

  @override
  String get myBookings => 'నా బుకింగ్స్';

  @override
  String get viewAll => 'అన్నీ చూడండి';

  @override
  String get accept => 'అంగీకరించండి';

  @override
  String get upcoming => 'రాబోయేవి';

  @override
  String get accepted => 'అంగీకరించబడింది';

  @override
  String get ongoing => 'జరుగుతోంది';

  @override
  String get completed => 'పూర్తయింది';

  @override
  String get cancelled => 'రద్దు చేయబడింది';

  @override
  String get exitAppQuestion => 'మీరు యాప్ నుండి బయటకు వెళ్లాలనుకుంటున్నారా?';

  @override
  String get exit => 'బయటకు వెళ్లండి';

  @override
  String get cancel => 'రద్దు చేయండి';

  @override
  String get cannotAcceptBooking => 'బుకింగ్‌ను అంగీకరించలేరు';

  @override
  String get turnOnlineFirst => 'బుకింగ్‌లను అంగీకరించడానికి దయచేసి ముందుగా ఆన్‌లైన్‌లోకి వెళ్లండి.';

  @override
  String get ok => 'సరే';

  @override
  String get confirmRide => 'రైడ్‌ను నిర్ధారించండి';

  @override
  String get acceptRideQuestion => 'మీరు నిజంగా ఈ రైడ్‌ను అంగీకరించాలనుకుంటున్నారా?';

  @override
  String get no => 'లేదు';

  @override
  String get yes => 'అవును';

  @override
  String get rideAccepted => 'రైడ్ అంగీకరించబడింది';

  @override
  String get rideAcceptedBy => 'మీ రైడ్ అంగీకరించబడింది';

  @override
  String get rideAcceptedSuccess => 'రైడ్ విజయవంతంగా అంగీకరించబడింది!';

  @override
  String get bookingNotFound => 'బుకింగ్ కనబడలేదు.';

  @override
  String get rideAlreadyTaken => 'రైడ్ ఇప్పటికే తీసుకోబడింది';

  @override
  String get rideAlreadyTakenByOther => 'ఈ రైడ్ ఇప్పటికే మరొక డ్రైవర్ ద్వారా అంగీకరించబడింది.';

  @override
  String get failedToUpdateStatus => 'స్థితిని నవీకరించడంలో విఫలమైంది:';

  @override
  String get confirmOnlineStatus => 'మీరు నిజంగా కావాలనుకుంటున్నారా';

  @override
  String get yourBookings => 'మీ బుకింగ్స్';

  @override
  String get noAcceptedBookings => 'బుకింగ్స్ అందుబాటులో లేవు';

  @override
  String get driver => 'డ్రైవర్';

  @override
  String get documents => 'పత్రాలు';

  @override
  String get helpSupport => 'సహాయం & మద్దతు';

  @override
  String get termsConditions => 'నిబంధనలు & షరతులు';

  @override
  String get privacyPolicy => 'గోప్యతా విధానం';

  @override
  String get logoutQuestion => 'మీరు నిజంగా లాగ్ అవుట్ కావాలనుకుంటున్నారా?';

  @override
  String get totalEarnings => 'మొత్తం ఆదాయం';

  @override
  String get ridePaymentNote => 'కస్టమర్ బుకింగ్‌కి నగదు చెల్లించిన తర్వాత రైడ్ చెల్లింపు ఆటోమేటిక్‌గా మీ ఖాతాలో జమ అవుతుంది.';

  @override
  String get noTransactionsFound => 'లావాదేవీలు కనబడలేదు';

  @override
  String get transactions => 'లావాదేవీలు';

  @override
  String get basicDetails => 'ప్రాథమిక వివరాలు';

  @override
  String get bankDetails => 'బ్యాంక్ వివరాలు';

  @override
  String get dateOfBirth => 'పుట్టిన తేదీ';

  @override
  String get light => 'లైట్';

  @override
  String get lightPremium => 'లైట్-ప్రీమియం';

  @override
  String get lightPremiumHeavy => 'లైట్-ప్రీమియం-హెవీ';

  @override
  String get chooseVehicleType => 'వాహన రకాన్ని ఎంచుకోండి';

  @override
  String get licenceNumber => 'లైసెన్స్ నంబర్';

  @override
  String get drivingLicenceFrontBack => ' డ్రైవింగ్ లైసెన్స్ (ముందు & వెనుక)';

  @override
  String get frontSide => 'ముందు భాగం';

  @override
  String get backSide => 'వెనుక భాగం';

  @override
  String get aadharFrontBack => ' ఆధార్ కార్డు (ముందు & వెనుక)';

  @override
  String get ifsc => 'IFSC';

  @override
  String get invalidIfscFormat => ' చెల్లని IFSC కోడ్ ఫార్మాట్.';

  @override
  String get ifscMaxLength => 'IFSC కోడ్ 11 అక్షరాలను మించకూడదు.';

  @override
  String get bankName => 'బ్యాంక్ పేరు';

  @override
  String get branch => 'శాఖ';

  @override
  String get accountNumber => 'ఖాతా నంబర్';

  @override
  String get accountHolderName => 'ఖాతాదారుడి పేరు';

  @override
  String get selectImageFrom => 'చిత్రాన్ని ఎంచుకోండి';

  @override
  String get camera => 'కెమెరా';

  @override
  String get gallery => 'గ్యాలరీ';

  @override
  String get rideDetails => 'రైడ్ వివరాలు';

  @override
  String get chat => 'చాట్';

  @override
  String get unableToOpenDialer => 'డయలర్ తెరవలేకపోయాం.';

  @override
  String get ownerPhoneUnavailable => 'యజమాని ఫోన్ నంబర్ అందుబాటులో లేదు.';

  @override
  String get call => 'కాల్';

  @override
  String get routeInformation => 'మార్గ సమాచారం';

  @override
  String get dropLocation1 => 'డ్రాప్ స్థానం 1';

  @override
  String get eta => 'అంచనా సమయం: ';

  @override
  String get distance => 'దూరం';

  @override
  String get duration => 'వ్యవధి';

  @override
  String get unableToOpenMaps => 'గూగుల్ మ్యాప్స్ తెరవలేకపోయాం.';

  @override
  String get errorOpeningDirections => 'దిశలు తెరవడంలో లోపం: ';

  @override
  String get getDirections => 'దిశలు పొందండి';

  @override
  String get tripDetails => 'ప్రయాణ వివరాలు';

  @override
  String get cityLimit => 'నగర పరిమితి: ';

  @override
  String get hours => 'గంటలు';

  @override
  String get slotDetails => 'స్లాట్ వివరాలు';

  @override
  String get departure => 'ప్రస్థానం';

  @override
  String get arrival => 'వచ్చిన సమయం';

  @override
  String get ownerDetails => 'యజమాని వివరాలు';

  @override
  String get paymentSummary => 'చెల్లింపు సారాంశం';

  @override
  String get servicePrice => 'సేవ ధర';

  @override
  String get convenienceFee => 'సౌకర్య రుసుము';

  @override
  String get couponApplied => 'కూపన్ వర్తింపజేయబడింది';

  @override
  String get totalPrice => 'మొత్తం ధర';

  @override
  String get customerReview => 'కస్టమర్ సమీక్ష';

  @override
  String get rating => 'రేటింగ్: ';

  @override
  String get feedback => 'అభిప్రాయం: ';

  @override
  String get comment => 'వ్యాఖ్య: ';

  @override
  String get cancelRide => 'రైడ్ రద్దు చేయండి';

  @override
  String get cancelFree => 'మీరు ఈ రైడ్‌ను ఉచితంగా రద్దు చేయవచ్చు.\nమీరు ఖచ్చితంగా అనుకుంటున్నారా?';

  @override
  String get cancelCharge => 'ఇప్పుడు రద్దు చేస్తే ₹39 ఛార్జ్ అవుతుంది.\nమీరు కొనసాగించాలనుకుంటున్నారా?';

  @override
  String get pay39 => '₹39.00 చెల్లించండి';

  @override
  String get cancelMyRide => 'నా రైడ్‌ను రద్దు చేయండి';

  @override
  String get rideCancelled => 'రైడ్ రద్దు చేయబడింది';

  @override
  String get waitingForPayment => 'చెల్లింపుకు వేచి ఉంది';

  @override
  String get paymentReceived => 'చెల్లింపు అందింది';

  @override
  String get paymentFailed => 'చెల్లింపు విఫలమైంది';

  @override
  String get waitingPaymentConfirmation => 'చెల్లింపు నిర్ధారణ కోసం వేచి ఉంది...';

  @override
  String get swipeCompleteRide => 'రైడ్ పూర్తి చేయడానికి స్వైప్ చేయండి';

  @override
  String get swipeStartRide => 'రైడ్ ప్రారంభించడానికి స్వైప్ చేయండి';

  @override
  String get completeRide => 'రైడ్ పూర్తి చేయండి';

  @override
  String get confirmCompleteRide => 'మీరు నిజంగా ఈ రైడ్‌ను పూర్తి చేయాలనుకుంటున్నారా?';

  @override
  String get rideCompleted => 'రైడ్ పూర్తయింది';

  @override
  String get rideCompletedMessage => 'మీ రైడ్ పూర్తయింది. Nyzo Ride ఎంచుకున్నందుకు ధన్యవాదాలు!';

  @override
  String get rideCompletedSuccess => 'రైడ్ విజయవంతంగా పూర్తయింది!';

  @override
  String get cancellation => 'రద్దు';

  @override
  String get cancelFreeShort => 'మీరు ఈ రైడ్‌ను ఉచితంగా రద్దు చేయవచ్చు.';

  @override
  String get cancelChargeShort => 'ఇప్పుడు రద్దు చేస్తే ₹39 ఛార్జ్ అవుతుంది.';

  @override
  String get proceed => 'కొనసాగించండి';

  @override
  String get enterOtp4 => '4-అంకెల OTP నమోదు చేయండి';

  @override
  String get invalidOtp4 => 'దయచేసి సరైన 4-అంకెల OTP నమోదు చేయండి.';

  @override
  String get rideStarted => 'రైడ్ ప్రారంభించబడింది';

  @override
  String get rideStartedMessage => 'మీ రైడ్ ప్రారంభించబడింది. సురక్షితమైన ప్రయాణం చేయండి!';

  @override
  String get rideStartedSuccess => 'రైడ్ విజయవంతంగా ప్రారంభించబడింది — సురక్షిత డ్రైవ్!';

  @override
  String get errorUpdatingStatus => 'స్థితిని నవీకరించడంలో లోపం:';

  @override
  String get confirm => 'నిర్ధారించండి';

  @override
  String get drivingLicence => 'డ్రైవింగ్ లైసెన్స్';

  @override
  String get uploaded => 'అప్‌లోడ్ చేయబడింది';

  @override
  String get notUploaded => 'అప్‌లోడ్ చేయబడలేదు';

  @override
  String get aadharCard => 'ఆధార్ కార్డు';

  @override
  String get noImageAvailable => 'చిత్రం అందుబాటులో లేదు';

  @override
  String get failedToLoadImage => 'చిత్రాన్ని లోడ్ చేయడంలో విఫలమైంది';

  @override
  String get privacyPolicyTitle => 'NYZO RIDE – డ్రైవర్ (కెప్టెన్)';

  @override
  String get lastUpdated => 'చివరి నవీకరణ: జనవరి 2026';

  @override
  String get privacyIntro => 'Nyzo Ride (“Nyzo Ride”, “మేము”, “మా”) మీ గోప్యతను గౌరవిస్తుంది మరియు Nyzo Ride మొబైల్ అప్లికేషన్, వెబ్‌సైట్ మరియు సంబంధిత సేవలను (“ప్లాట్‌ఫారమ్”) ఉపయోగించే వినియోగదారులు (“మీరు”, “వినియోగదారు”, “డ్రైవర్”, “వాహన యజమాని”) యొక్క వ్యక్తిగత సమాచారాన్ని రక్షించడానికి కట్టుబడి ఉంది.';

  @override
  String get privacyPolicyExplanation => 'ఈ గోప్యతా విధానం మేము మీ సమాచారాన్ని ఎలా సేకరిస్తాము, ఉపయోగిస్తాము, నిల్వ చేస్తాము, పంచుకుంటాము మరియు రక్షిస్తాము అనేది వివరిస్తుంది.';

  @override
  String get informationWeCollect => 'మేము సేకరించే సమాచారం';

  @override
  String get personalInformation => '1.1 వ్యక్తిగత సమాచారం';

  @override
  String get weMayCollectFollowing => 'మేము క్రింది సమాచారాన్ని సేకరించవచ్చు:';

  @override
  String get fullName => 'పూర్తి పేరు';

  @override
  String get emailOptional => 'ఇమెయిల్ చిరునామా (ఐచ్చికం)';

  @override
  String get profilePhotoOptional => 'ప్రొఫైల్ ఫోటో (ఐచ్చికం)';

  @override
  String get governmentIdDetails => 'ప్రభుత్వ ID వివరాలు (డ్రైవర్ల కోసం – లైసెన్స్, PAN/ఆధార్ అవసరమైన చోట)';

  @override
  String get vehicleDetails => 'వాహన వివరాలు (వాహన యజమానులు)';

  @override
  String get locationInformation => '1.2 స్థానం సమాచారం';

  @override
  String get realTimeLocation => 'క్రియాశీల ప్రయాణాల సమయంలో రియల్-టైమ్ స్థానం';

  @override
  String get pickupDropLocations => 'పికప్ మరియు డ్రాప్ స్థానాలు';

  @override
  String get tripRoutesDistance => 'ప్రయాణ మార్గాలు మరియు దూరం డేటా';

  @override
  String get locationCollectionNote => 'యాప్ ఉపయోగంలో ఉన్నప్పుడు మరియు ప్రయాణ సంబంధిత ప్రయోజనాల కోసం మాత్రమే స్థానం సేకరించబడుతుంది.';

  @override
  String get usageDeviceInformation => '1.3 వినియోగం & పరికర సమాచారం';

  @override
  String get deviceModel => 'పరికర మోడల్';

  @override
  String get osVersion => 'ఆపరేటింగ్ సిస్టమ్ వెర్షన్';

  @override
  String get appUsageData => 'యాప్ వినియోగ డేటా';

  @override
  String get ipAddress => 'IP చిరునామా';

  @override
  String get crashLogs => 'క్రాష్ లాగ్స్ మరియు డయాగ్నస్టిక్ డేటా';

  @override
  String get paymentInformation => '1.4 చెల్లింపు సమాచారం';

  @override
  String get paymentStatus => 'చెల్లింపు స్థితి';

  @override
  String get transactionId => 'లావాదేవీ ID';

  @override
  String get paymentMethodNote => 'చెల్లింపు విధానం (Nyzo Ride కార్డ్ లేదా UPI వివరాలను నిల్వ చేయదు)';

  @override
  String get howWeUseInformation => '2. మేము మీ సమాచారాన్ని ఎలా ఉపయోగిస్తాము';

  @override
  String get weUseYourInformationTo => 'మేము మీ సమాచారాన్ని ఉపయోగిస్తాము:';

  @override
  String get connectOwnersDrivers => 'వాహన యజమానులను డ్రైవర్లతో కలపడం';

  @override
  String get enableBookingsTrips => 'బుకింగ్స్ మరియు ప్రయాణ నిర్వహణను ప్రారంభించడం';

  @override
  String get verifyDriverIdentity => 'డ్రైవర్ గుర్తింపు మరియు అర్హతను ధృవీకరించడం';

  @override
  String get processPaymentsRefunds => 'చెల్లింపులు మరియు రీఫండ్‌లను ప్రాసెస్ చేయడం';

  @override
  String get improveAppPerformance => 'యాప్ పనితీరు మరియు వినియోగదారు అనుభవాన్ని మెరుగుపరచడం';

  @override
  String get communicateUpdatesAlerts => 'సేవా నవీకరణలు మరియు అలర్ట్‌లను తెలియజేయడం';

  @override
  String get preventFraudMisuse => 'మోసం మరియు దుర్వినియోగాన్ని నివారించడం';

  @override
  String get complyLegalObligations => 'చట్టపరమైన బాధ్యతలను అనుసరించడం';

  @override
  String get informationSharing => '3. సమాచారం పంచుకోవడం';

  @override
  String get noSellRentData => 'Nyzo Ride మీ వ్యక్తిగత డేటాను అమ్మదు లేదా అద్దెకు ఇవ్వదు.';

  @override
  String get weMayShareLimitedInfo => 'మేము పరిమిత సమాచారాన్ని పంచుకోవచ్చు:';

  @override
  String get shareDriversOwners => 'డ్రైవర్లు / వాహన యజమానులు – ప్రయాణానికి అవసరమైన సమాచారం మాత్రమే';

  @override
  String get sharePaymentGateways => 'చెల్లింపు గేట్వేలు – లావాదేవీ ప్రాసెసింగ్ కోసం';

  @override
  String get shareCloudPartners => 'క్లౌడ్ & టెక్నాలజీ భాగస్వాములు – యాప్ హోస్టింగ్, విశ్లేషణ';

  @override
  String get shareLawAuthorities => 'చట్ట అమలు / ప్రభుత్వ అధికారులు – చట్టపరంగా అవసరమైనప్పుడు';

  @override
  String get dataStorageSecurity => '4. డేటా నిల్వ & భద్రత';

  @override
  String get dataStoredSecureServers => 'డేటా సురక్షిత సర్వర్లలో నిల్వ చేయబడుతుంది';

  @override
  String get industryEncryptionUsed => 'పరిశ్రమ-ప్రామాణిక గుప్తీకరణ ఉపయోగించబడుతుంది';

  @override
  String get accessLimitedAuthorized => 'ప్రవేశం కేవలం అధికృత సిబ్బందికి మాత్రమే పరిమితం';

  @override
  String get regularSecurityAudits => 'నియమిత భద్రతా ఆడిట్‌లు నిర్వహించబడతాయి';

  @override
  String get noSystemFullySecure => 'ఉత్తమ ప్రయత్నాలున్నప్పటికీ, ఏ డిజిటల్ వ్యవస్థ 100% సురక్షితం కాదు. వినియోగదారులు తమ స్వంత ప్రమాదంతో డేటాను పంచుకుంటారు.';

  @override
  String get userRights => '5. వినియోగదారుల హక్కులు';

  @override
  String get youHaveTheRightTo => 'మీకు హక్కు ఉంది:';

  @override
  String get accessPersonalData => 'మీ వ్యక్తిగత డేటాను యాక్సెస్ చేయండి';

  @override
  String get updateInformation => 'మీ సమాచారాన్ని నవీకరించండి లేదా సరిదిద్దండి';

  @override
  String get requestAccountDeletion => 'మీ ఖాతాను తొలగించమని అభ్యర్థించండి';

  @override
  String get withdrawConsent => 'సమ్మతి ఉపసంహరించుకోండి (అన్వయించబడితే)';

  @override
  String get accountDeletionNote => 'ఖాతా తొలగింపు అభ్యర్థనలు చట్టపరమైన లేదా నియంత్రణ పరిరక్షణ అవసరాలకు లోబడి ఉండవచ్చు.';

  @override
  String get dataRetention => '6. డేటా నిల్వ';

  @override
  String get dataRetainedAsNecessary => 'డేటా అవసరమైనంత కాలం మాత్రమే నిల్వ చేయబడుతుంది';

  @override
  String get tripTransactionRetention => 'ప్రయాణం మరియు లావాదేవీ రికార్డులు చట్ట, పన్ను లేదా వివాద ప్రయోజనాల కోసం నిల్వ చేయబడవచ్చు';

  @override
  String get inactiveAccountsDeleted => 'క్రియాశీలం కాని ఖాతాలు నిర్వచిత కాలం తర్వాత తొలగించబడవచ్చు';

  @override
  String get childrensPrivacy => '7. పిల్లల గోప్యత';

  @override
  String get notForUnder18 => 'Nyzo Ride 18 సంవత్సరాల లోపు వినియోగదారుల కోసం ఉద్దేశించబడలేదు';

  @override
  String get noMinorDataCollection => 'మేము చిన్నారుల నుండి డేటాను ఉద్దేశపూర్వకంగా సేకరించము';

  @override
  String get thirdPartyServices => '8. తృతీయ పక్ష సేవలు';

  @override
  String get mayContainThirdPartyLinks => 'Nyzo Ride లో తృతీయ పక్ష సేవల లింకులు లేదా ఇంటిగ్రేషన్లు ఉండవచ్చు';

  @override
  String get notResponsibleThirdPartyPrivacy => 'మేము వారి గోప్యతా విధానాలకు బాధ్యత వహించము';

  @override
  String get platformDisclaimer => '9. ప్లాట్‌ఫారమ్ స్వభావ నిరాకరణ';

  @override
  String get platformOnly => 'Nyzo Ride కేవలం ఒక సాంకేతిక ప్లాట్‌ఫారమ్ మాత్రమే:';

  @override
  String get notTransportOperator => 'Nyzo Ride రవాణా ఆపరేటర్ కాదు';

  @override
  String get doesNotOwnVehicles => 'Nyzo Ride వాహనాలను కలిగి లేదు';

  @override
  String get doesNotEmployDrivers => 'Nyzo Ride డ్రైవర్లను నియమించదు';

  @override
  String get independentProviders => 'డ్రైవర్లు మరియు వాహన యజమానులు స్వతంత్ర సేవా ప్రదాతలు';

  @override
  String get policyChanges => '10. ఈ విధానంలో మార్పులు';

  @override
  String get policyMayUpdate => 'Nyzo Ride ఈ గోప్యతా విధానాన్ని సమయానుకూలంగా నవీకరించవచ్చు';

  @override
  String get policyUpdatedVersions => 'నవీకరించిన వెర్షన్లు యాప్ లేదా వెబ్‌సైట్‌లో ప్రచురించబడతాయి';

  @override
  String get policyContinuedUse => 'ప్లాట్‌ఫారమ్‌ను నిరంతరం ఉపయోగించడం సవరించిన విధానాన్ని అంగీకరించినట్లే';

  @override
  String get contactUs => '11. మమ్మల్ని సంప్రదించండి';

  @override
  String get privacyConcernsContact => 'గోప్యతా సమస్యలు లేదా డేటా అభ్యర్థనల కోసం సంప్రదించండి:';

  @override
  String get nyzoRideSupport => 'Nyzo Ride సహాయం';

  @override
  String get nyzoRideEmail => 'hello@nyzoride.com';

  @override
  String get nyzoRidePhone => '9000464851';

  @override
  String get driverTermsTitle => 'NYZO RIDE – డ్రైవర్ (కెప్టెన్)';

  @override
  String get driverEligibility => '1. డ్రైవర్ అర్హత';

  @override
  String get driverEligibilityAge => 'డ్రైవర్ కనీసం 21 సంవత్సరాల వయస్సు కలిగి ఉండాలి.';

  @override
  String get driverEligibilityLicense => 'చెల్లుబాటు అయ్యే డ్రైవింగ్ లైసెన్స్ (LMV / HMV అవసరమైతే) తప్పనిసరి.';

  @override
  String get driverEligibilityDocuments => 'సమర్పించిన అన్ని పత్రాలు నిజమైనవి మరియు చెల్లుబాటు అయ్యేవి కావాలి.';

  @override
  String get platformNature => '2. ప్లాట్‌ఫారమ్ స్వభావం';

  @override
  String get platformNatureDesc1 => 'Nyzo Ride కేవలం ఒక సాంకేతిక ప్లాట్‌ఫారమ్ మాత్రమే.';

  @override
  String get platformNatureDesc2 => 'ఇది వాహన యజమానులు మరియు డ్రైవర్లను కలుపుతుంది.';

  @override
  String get platformNatureNotOwner => 'Nyzo Ride వాహన యజమాని కాదు.';

  @override
  String get platformNatureNotDriver => 'Nyzo Ride డ్రైవర్ కాదు.';

  @override
  String get platformNatureNotOperator => 'Nyzo Ride రవాణా ఆపరేటర్ కాదు.';

  @override
  String get noJobGuarantee => '3. ఉద్యోగం లేదా ఉపాధి హామీ లేదు';

  @override
  String get nyzoNoJobs => 'Nyzo Ride ఉద్యోగాలు లేదా ఉపాధి అందించదు.';

  @override
  String get driversNotEmployees => 'డ్రైవర్లు Nyzo Ride ఉద్యోగులు కాదు.';

  @override
  String get tripAvailabilityNotGuaranteed => 'ప్రయాణ లభ్యతకు హామీ లేదు.';

  @override
  String get nyzoNoFixedIncome => 'Nyzo Ride స్థిరమైన ఆదాయం, జీతం లేదా కనీస ప్రయాణాలను హామీ ఇవ్వదు.';

  @override
  String get independentDriverStatus => '4. స్వతంత్ర డ్రైవర్ స్థితి';

  @override
  String get driversIndependentProviders => 'డ్రైవర్లు స్వతంత్ర సేవా ప్రదాతలుగా వ్యవహరిస్తారు.';

  @override
  String get noEmployerEmployeeRelation => 'ఏ నియోజకుడు–ఉద్యోగి సంబంధం ఏర్పడదు.';

  @override
  String get noBenefitsApplicable => 'PF, ESI, సెలవు లేదా బీమా వంటి ప్రయోజనాలు వర్తించవు.';

  @override
  String get taxComplianceDriver => 'పన్ను మరియు చట్టపరమైన అనుసరణ డ్రైవర్ బాధ్యత.';

  @override
  String get preRideInspection => '5. ప్రయాణానికి ముందు వాహన తనిఖీ';

  @override
  String get driverMustInspectVehicle => 'ప్రతి ప్రయాణానికి ముందు, డ్రైవర్ వాహనాన్ని తనిఖీ చేయాలి.';

  @override
  String get inspectionChecklist => 'టైర్లు, బ్రేకులు, లైట్లు, అద్దాలు, ఇంధన స్థాయి మరియు పత్రాలను తనిఖీ చేయాలి.';

  @override
  String get unsafeVehicleNoTrip => 'వాహనం సురక్షితం కాకపోతే, డ్రైవర్ ప్రయాణాన్ని ప్రారంభించకూడదు మరియు యజమానికి తెలియజేయాలి.';

  @override
  String get nyzoNotResponsibleVehicle => 'Nyzo Ride వాహన స్థితి లేదా సంబంధిత సమస్యలకు బాధ్యత వహించదు.';

  @override
  String get tripResponsibilities => '6. ప్రయాణ బాధ్యతలు';

  @override
  String get driverOnTimePickup => 'డ్రైవర్ సమయానికి పికప్ స్థలానికి చేరుకోవాలి.';

  @override
  String get followTrafficRules => 'అన్ని ట్రాఫిక్ నియమాలు మరియు స్థానిక చట్టాలను పాటించాలి.';

  @override
  String get driveCarefullyResponsibly => 'యజమాని వాహనాన్ని జాగ్రత్తగా మరియు బాధ్యతతో నడపాలి.';

  @override
  String get misbehaviorProhibited => 'దుర్వ్యవహారం, అసురక్షిత డ్రైవింగ్ లేదా నిర్లక్ష్యం ఖచ్చితంగా నిషేధించబడింది.';

  @override
  String get cancellationPolicy => '7. రద్దు విధానం';

  @override
  String get avoidUnnecessaryCancellations => 'డ్రైవర్ అనవసరమైన రద్దులను నివారించాలి.';

  @override
  String get frequentCancellationsLeadTo => 'తరచుగా రద్దు చేస్తే:';

  @override
  String get cancellationWarning => 'హెచ్చరిక';

  @override
  String get cancellationTemporarySuspension => 'తాత్కాలిక సస్పెన్షన్';

  @override
  String get cancellationPermanentDeactivation => 'శాశ్వత ఖాతా డీయాక్టివేషన్';

  @override
  String get paymentsEarnings => '8. చెల్లింపులు & ఆదాయం';

  @override
  String get earningsAfterCompletion => 'ఆదాయం విజయవంతమైన ప్రయాణం పూర్తయిన తర్వాత మాత్రమే జమ అవుతుంది.';

  @override
  String get serviceChargesPolicy => 'Nyzo Ride విధానం ప్రకారం సేవా ఛార్జీలు వర్తించవచ్చు.';

  @override
  String get noPaymentFraudTrips => 'నకిలీ లేదా మోసపూరిత ప్రయాణాలకు చెల్లింపు చేయబడదు.';

  @override
  String get ratingsReviews => '9. రేటింగ్‌లు & సమీక్షలు';

  @override
  String get driverPerformanceEvaluated => 'డ్రైవర్ పనితీరు రేటింగ్‌లు మరియు అభిప్రాయం ఆధారంగా అంచనా వేయబడుతుంది.';

  @override
  String get lowRatingsSuspension => 'తరచుగా తక్కువ రేటింగ్‌లు లేదా ఫిర్యాదులు సస్పెన్షన్ లేదా రద్దుకు దారితీస్తాయి.';

  @override
  String get prohibitedActivities => '10. నిషేధిత కార్యకలాపాలు';

  @override
  String get noAlcoholDrugsDriving => 'మద్యం లేదా మాదకద్రవ్యాల ప్రభావంలో వాహనం నడపడం ఖచ్చితంగా నిషేధించబడింది.';

  @override
  String get noFakeDocuments => 'నకిలీ పత్రాలను అప్‌లోడ్ చేయడం అనుమతించబడదు.';

  @override
  String get noAppMisuseFraud => 'యాప్ దుర్వినియోగం, మోసం, దుర్వినియోగం లేదా వేధింపులు నిషేధించబడ్డాయి.';

  @override
  String get noBrandMisuse => 'Nyzo Ride బ్రాండింగ్ దుర్వినియోగం అనుమతించబడదు.';

  @override
  String get insuranceLiability => '11. బీమా & బాధ్యత';

  @override
  String get insuranceCoverageProvided => 'బీమా కవరేజీ Nyzo Ride ప్రత్యేకంగా అందించినప్పుడు మాత్రమే వర్తిస్తుంది.';

  @override
  String get driverResponsibleAccidents => 'డ్రైవర్ ప్రమాదాలు, జరిమానాలు, శిక్షలు మరియు ఉల్లంఘనలకు బాధ్యత వహించాలి.';

  @override
  String get nyzoNotLiableIncidents => 'Nyzo Ride ఏవైనా సంఘటనలకు నేరుగా బాధ్యత వహించదు.';

  @override
  String get accountSuspensionTermination => '12. ఖాతా సస్పెన్షన్ లేదా రద్దు';

  @override
  String get nyzoSuspendTerminateAccounts => 'Nyzo Ride నియమ ఉల్లంఘనల కోసం డ్రైవర్ ఖాతాలను సస్పెండ్ లేదా రద్దు చేసే హక్కు కలిగి ఉంది.';

  @override
  String get immediateActionLegalIssues => 'చట్టపరమైన సమస్యలు లేదా పోలీస్ ఫిర్యాదుల సందర్భంలో తక్షణ చర్య తీసుకోబడుతుంది.';

  @override
  String get dataPrivacy => '13. డేటా & గోప్యత';

  @override
  String get driverDataServicePurpose => 'డ్రైవర్ వ్యక్తిగత డేటా కేవలం సేవా సంబంధిత ప్రయోజనాల కోసం మాత్రమే ఉపయోగించబడుతుంది.';

  @override
  String get dataSharedByLaw => 'డేటా చట్టం ద్వారా అవసరమైనప్పుడు మాత్రమే మూడవ పక్షాలతో పంచబడుతుంది.';

  @override
  String get changesToTerms => '14. నిబంధనల్లో మార్పులు';

  @override
  String get nyzoModifyTerms => 'Nyzo Ride ఎప్పుడైనా ఈ నిబంధనలు & షరతులను మార్చవచ్చు.';

  @override
  String get updatesCommunicatedApp => 'నవీకరణలు యాప్ ద్వారా తెలియజేయబడతాయి.';

  @override
  String get legalJurisdiction => '15. చట్టపరమైన పరిధి';

  @override
  String get termsGovernedIndianLaw => 'ఈ నిబంధనలు భారతీయ చట్టాల ప్రకారం అమలు చేయబడతాయి.';

  @override
  String get disputesHyderabadJurisdiction => 'ఏవైనా వివాదాలు హైదరాబాద్ పరిధిలో పరిష్కరించబడతాయి.';

  @override
  String get acceptance => '16. అంగీకారం';

  @override
  String get driverAcceptanceConfirm => 'Nyzo Ride డ్రైవర్ యాప్‌ను ఉపయోగించడం ద్వారా, మీరు ధృవీకరిస్తున్నారు:';

  @override
  String get driverAcceptanceQuote => '“నేను పై నిబంధనలు & షరతులను చదివి, అర్థం చేసుకుని, అంగీకరించాను.”';

  @override
  String get captainTermsTitle => 'Nyzo Ride కెప్టెన్ నిబంధనలు & షరతులు';

  @override
  String get basicInformation => 'ప్రాథమిక సమాచారం';

  @override
  String get firstName => 'మొదటి పేరు';

  @override
  String get lastName => 'చివరి పేరు';

  @override
  String get email => 'ఇమెయిల్:';

  @override
  String get drivingLicenceNumber => 'డ్రైవింగ్ లైసెన్స్ నంబర్';

  @override
  String get preferences => 'అభిరుచులు';

  @override
  String get uploadDocuments => 'పత్రాలను అప్‌లోడ్ చేయండి';

  @override
  String get uploadDriverLicence => 'డ్రైవర్ లైసెన్స్ అప్‌లోడ్ చేయండి';

  @override
  String get uploadAadharCard => 'ఆధార్ కార్డ్ అప్‌లోడ్ చేయండి';

  @override
  String get uploadFront => 'ముందు వైపు అప్‌లోడ్ చేయండి';

  @override
  String get uploadBack => 'వెనుక వైపు అప్‌లోడ్ చేయండి';

  @override
  String get ifscCode => 'IFSC కోడ్';

  @override
  String get agreePrivacyPolicy => 'గోప్యతా విధానంలో పేర్కొన్న విధంగా నా సమాచారాన్ని సేకరించడం మరియు ఉపయోగించడాన్ని నేను అంగీకరిస్తున్నాను.';

  @override
  String get continueText => 'కొనసాగించండి';

  @override
  String get phone => 'ఫోన్:';

  @override
  String get rideId => 'రైడ్ ఐడి';

  @override
  String get helpDesc => 'రైడ్స్, చెల్లింపులు, ఖాతా లేదా యాప్ వినియోగానికి సంబంధించిన ఏ సమస్యకైనా మేము మీకు సహాయం చేయడానికి సిద్ధంగా ఉన్నాము.';

  @override
  String get quickHelp => 'త్వరిత సహాయ ఎంపికలు';

  @override
  String get chooseWay => 'మమ్మల్ని సంప్రదించడానికి సరైన మార్గాన్ని ఎంచుకోండి:';

  @override
  String get callSupport => 'కాల్స్ సహాయం';

  @override
  String get callDesc => 'మా సహాయ బృందం నుండి వెంటనే సహాయం పొందండి';

  @override
  String get availableHours => 'పని సమయాల్లో అందుబాటులో ఉంటుంది';

  @override
  String get emailSupport => 'ఈమెయిల్ సహాయం';

  @override
  String get emailDesc => 'మీ ప్రశ్నలు లేదా సమస్యలను మాకు పంపండి';

  @override
  String get emailReplyTime => 'మేము సాధారణంగా 24 గంటల్లో స్పందిస్తాము';
}
